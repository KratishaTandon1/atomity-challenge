  ...
  return (
    <section className="w-full max-w-5xl mx-auto py-8 sm:py-16 px-4 sm:px-6 lg:px-8">
      <div className="bg-surface rounded-2xl sm:rounded-[var(--radius-card)] shadow border border-border-subtle p-6 sm:p-10 relative overflow-hidden">
        ...
        {/* --- Bar Chart Section --- */}
        <div className="relative h-64 border-b border-dashed border-border-subtle mb-12 pb-4 flex justify-center items-end gap-12 sm:gap-24">
          {isLoading ? (
            ...
            <AnimatePresence mode="wait">
              {entities?.map((entity) => (
                ...
                >
                  ...
                  <span className="mt-4 text-sm font-semibold text-foreground tracking-tight whitespace-nowrap">
                    {entity.name}
                  </span>
                </motion.div>
              ))}
